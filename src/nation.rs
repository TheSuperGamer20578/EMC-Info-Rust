...
pub fn get(data: &Data, nation: &String) -> Result<Nation> {
    let name = if data.ignore_case { nation.to_lowercase() } else { nation };
    let mut nation_name: Option<&str> = None;
    for (town_name, town) in data.towns.iter() {
        ...
        if if data.ignore_case { town_nation.to_lowercase() } else { town_nation } != name {
            continue;
        }
